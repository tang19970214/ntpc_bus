...
export default {
  ...
  methods: {
    ...
    async getBusInfo(row) {
      ...
      await this.getDistanceTime(this.Point1, this.Point2)
      let getBusTime = this.duration
    ...
    async setMarker(getBusLocal) {
      ...
      await allList.forEach((location, idx) => {
        let getInfos = this.busList.filter(
          (res) => getBusInfos.includes(res.carId) || {}
        )

        this.Point1 = {
          lat: Number(this.latMarker),
          lng: Number(this.lonMarker),
        }
        this.Point2 = {
        ...
        // let tootTipTime = this.duration
        // console.log("tootTipTime", tootTipTime)
    ...
    async getDistanceTime(Point1, Point2) {
      // console.log(Point1, Point2)
      // return
      let directionsService = new google.maps.DirectionsService()
      const points = {
        origin: Point1, // 自訂點位
        destination: Point2, //點選巴士點位
        travelMode: "DRIVING", // 交通方式（開車)
      }
      // console.log();

      directionsService.route(points, (response, status) => {
        if (status == "OK") {
          // setTimeout(() => {
          ...
          // return
          // setTimeout(() => {
          // this.getDistanceTime(Point1, Point2)
          // }, 1500)
        } else {
